...
public class WeightedScoringStrategy : IScoringStrategy
{
    ...
    public UniTask<int> Score()
    {
        var dealerSprite = _dealer.sprite;
        ...
        if (dealerScore >= p2Score && dealerScore >= p1Score) {
            if (p1Score > p2Score) {
                overallScore = 1;
                LogManager.Instance.Log(new LogData("Neither player has bested me, but you have the higher score. 1 point.", "Dealer", dealerSprite));
            }
            else if (p2Score > p1Score) {
                LogManager.Instance.Log(new LogData("Neither player has bested me, and you have the lower score. -1 point.", "Dealer", dealerSprite));
                overallScore = -1;
            }
        }
        ...
        //Both players beat the dealer
            ...
            if (p1Score > p2Score) {
                LogManager.Instance.Log(new LogData("Both bested me this round but it is you who takes this hand. 2 points.", "Dealer", dealerSprite));
                overallScore = 2;
            }
            else if (p2Score > p1Score) {
                LogManager.Instance.Log(new LogData("You've both exceeded my score, but the opponent exceeds yours. -2 points.", "Dealer", dealerSprite));
                overallScore = -2;
            }
        }
        ...
        //Only 1 player beats the dealer
            ...
            if (p1Score > p2Score) {
                LogManager.Instance.Log(new LogData("Only you had the hand to beat mine. 3 points.", "Dealer", dealerSprite));
                overallScore = 3;
            }
            else if (p2Score > p1Score) {
                LogManager.Instance.Log(new LogData("Your opponent trumps my hand and yours. -3 points.", "Dealer", dealerSprite));
                overallScore = -3;
            }
        }
        
        if (overallScore == 0){
            LogManager.Instance.Log(new LogData("A tie. No points.", "Dealer", dealerSprite));
        }
